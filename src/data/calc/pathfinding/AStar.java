	...
	public static void setField(int[][] field){
		...
		for (int i = 0; i < field.length; i++){
			...
			for (int j = 0; j < field[i].length; j++){
				AStar.field.get(i).add(new Node(NodeData.collisionData[field[i][j]], i, j));
			}
		}
	}
	
	public static void path(Point start, Point end){
		AStar.start = field.get(start.x).get(start.y);
		AStar.end = field.get(end.x).get(end.y);
		calculate();
	}
	
	public static void area(Point center, int r){
		start = field.get(center.x).get(center.y);
		range = r;
		calculate();
	}
